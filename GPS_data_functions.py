...
def traj_process(location, start_end_time, folder_name, platoon_number):
    ...
    for location in divided_location:
        ...
        for tt in range(len(traj_with_coordinate[0])):
            ...
            v_num=0
            for veh in location:
                point.append((traj_with_coordinate[4+v_num*2][tt], traj_with_coordinate[5+v_num*2][tt]))
                v_num += 1
            if len(distance[0]) == 0:
                distance[0].append(0)
            else:
                distance[0].append(traj_with_coordinate[1][tt-1] * 0.44704 /data_frequency + distance[0][-1])
            distance[1].append(distance[0][-1]-cal_distance(point[0],point[1]))
            distance[2].append(distance[1][-1]-cal_distance(point[1],point[2]))
...
def speed_visulization(traj_dict, folder_name, MA_window = 2, overall=False):
    ...
    for traj in traj_dict:
        ...
        extended_period = 80
        divided_traj, extended_divided_traj = traj_by_oscillation_manual(traj, oscillation_set, extended_period)
        ...
        for traj in divided_traj:
            ...
            plt.title('OSCILLATION: '+str(split)+', HEADWAY: '+oscillation_set[traj_num][2]+
                      ', ENGINE MODE: '+oscillation_set[traj_num][3]+'\nMAGNITUDE: '+oscillation_set[traj_num][4]+
                      ', BRAKE PATTERN: '+oscillation_set[traj_num][5]+', CRUISE: '+oscillation_set[traj_num][6])
...
def read_oscillation_time(folder_name):
    ...
    while True:
        ...
        tmp = line.split(',')
        try:
            oscillation_set.append((int(tmp[0]), int(tmp[1]), tmp[6], tmp[8], tmp[9], tmp[10], tmp[11]))
            #start, end, headway, power mode, magnitude, brake pattern, cruise
        except:
            print('missing one', tmp[8], tmp[9], tmp[10], tmp[11])
    fo.close()
    return oscillation_set

def traj_by_oscillation_manual(traj, oscillation_set, extended_period=100):
    divided_traj=[]
